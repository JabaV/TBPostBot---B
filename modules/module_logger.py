...
Предоставляет две простые функции для записи логов:
- Log: информационные сообщения (log.txt)
- eLog: ошибки и предупреждения (error_log.txt)

Файлы логов открываются в режиме append и буфер сбрасывается после каждой записи,
что делает записи немедленно доступными для просмотра.
